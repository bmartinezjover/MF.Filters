...
export const useItem = function () {
  const getSelectedSection = computed(() => $_sections.find((x) => x.id === $_sectionIdSelected.value))
  ...
  const getSelectedItem = computed(() => getItems.value?.find((x) => x.id === $_itemIdSelected.value))
